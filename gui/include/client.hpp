...
public:
    ...
    /**
     * @brief Send MSG_NICK_SET to the server to change this client's nickname.
     * Updates m_nickname and g_config.nickname.
     */
    ...
    // When set, Client::print() calls this instead of writing to stdout.
    using MessageCallback = std::function<void(const std::string& msg, bool is_private, bool is_system)>;
    void set_message_callback(MessageCallback cb) { m_msg_cb = cb; }

    // Peer list callback (called when MSG_CLIENT_LIST is received)
    using PeerListCallback = std::function<void(const std::vector<std::pair<uint32_t,std::string>>&)>;
    void set_peer_list_callback(PeerListCallback cb) { m_peer_cb = cb; }
    // ------------------------------------------------------------------
...
private:
    ...
    MessageCallback     m_msg_cb;         ///< Optional GUI message callback.
    PeerListCallback    m_peer_cb;        ///< Optional GUI peer-list callback.
